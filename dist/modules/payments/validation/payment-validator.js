...
class PaymentValidator {
    ...
    async validatePaymentCallback(params) {
        ...
        if (params.signature) {
            const isValidSignature = await this.validateSignature(params.paymentId, params.status, params.amountCents || payment.amountCents, params.signature);
            if (!isValidSignature) {
    ...
    async validateRefund(params) {
        ...
        const totalRefunded = existingRefunds.reduce((sum, refund) => sum + (refund.amountCents || 0), 0);
     ...
     */
    validatePaymentMethodData(method, data) {
        const errors = [];
        const warnings = [];
        switch (method) {
            case 'credit_card':
    ...
    validateExpiryDate(expiryDate) {
        ...
        if (expYear < currentDate.getFullYear())
            return false;
        if (expYear === currentDate.getFullYear() && expMonth < currentMonth)
            return false;
        return true;
    }
    validateCVV(cvv) {
    ...
    async validateSignature(paymentId, status, amountCents, signature) {
