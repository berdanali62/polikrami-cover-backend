...
describe('API smoke tests', () => {
  it('Smoke 1: GET /health returns 200', async () => {
    const res = await request(app).get('/health');
    expect(res.status).toBe(200);
    expect(res.body).toHaveProperty('status', 'ok');
  });

  it('Smoke 2: GET /csrf sets csrf cookie', async () => {
  ...
  it('Smoke 3: GET /api/message-cards is public', async () => {
  ...
  it('Smoke 4: GET /api/users/me requires auth', async () => {
    const res = await request(app).get('/api/users/me');
    expect(res.status).toBe(401);
  });

  it('Smoke 5: GET /api/projects requires auth', async () => {
    const res = await request(app).get('/api/projects');
    expect(res.status).toBe(401);
  });

  it('Smoke 6: GET /api/drafts requires auth', async () => {
    const res = await request(app).get('/api/drafts');
    expect(res.status).toBe(401);
  });

  it('Smoke 7: GET /api/orders requires auth', async () => {
    const res = await request(app).get('/api/orders');
    expect(res.status).toBe(401);
  });

  it('Smoke 8: POST /api/auth/refresh without cookie returns 401', async () => {
