...
export async function recommendedDesignersController(_req: Request, res: Response) {
  ...
  res.status(200).json({ slate, rest });
}
...
export async function listDesignersSortedController(req: Request, res: Response) {
  ...
  if (sort === 'rating') {
    ...
    return res.status(200).json({ designers: sorted });
  }
  if (sort === 'newest') {
...
export async function createReviewController(req: Request, res: Response) {
  ...
  if (!isDesigner) return res.status(404).json({ message: 'Designer not found' });
  ...
  res.status(201).json({ id: review.id, rating: review.rating, comment: review.comment });
}
