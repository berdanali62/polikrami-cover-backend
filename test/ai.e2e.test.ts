...
describe('AI E2E (tolerant)', () => {
  ...
  beforeAll(async () => {
    ...
    await agent.post('/api/auth/register').set('X-CSRF-Token', String(csrf)).send({ email, password, confirmPassword: password, name: 'AI E2E' });
  ...
  it('GET /api/ai/templates returns templates', async () => {
    ...
    if (res.status === 200) expect(Array.isArray(res.body?.templates)).toBe(true);
  });
