...
export async function firebasePhoneVerifyController(req: Request, res: Response) {
  if (env.PHONE_VERIFY_PROVIDER !== 'firebase') return res.status(400).json({ message: 'Firebase provider disabled' });
  const userId = req.user?.id as string;
  const { idToken } = firebaseVerifySchema.parse(req.body);
  const decoded = await verifyFirebaseIdToken(idToken);
  if (!decoded || !decoded.phoneNumber) return res.status(400).json({ ok: false, message: 'Invalid Firebase token' });
  ...
  res.status(200).json({ ok: true, phone: decoded.phoneNumber });
}
