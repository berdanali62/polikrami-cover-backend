...
exports.DraftAssignmentService = void 0;
...
class DraftAssignmentService {
     ...
     */
    async assignDesigner(draftId, designerId, assignedBy) {
        const transactionManager = new transaction_1.TransactionManager(this.prisma);
        return transactionManager.executeWithIsolation(async (tx) => {
            ...
            if (draft.committedAt) {
                throw (0, ApiError_1.badRequest)('Cannot assign designer to committed draft');
            }
            if (draft.status !== 'ready') {
                throw (0, ApiError_1.badRequest)('Draft must be in ready status for assignment');
            }
            ...
            if (draft.assignedDesignerId) {
                ...
                throw (0, ApiError_1.badRequest)('Draft already has an assigned designer');
            }
            ...
            if (!designer || designer.roles.length === 0) {
                throw (0, ApiError_1.badRequest)('User is not a designer');
            }
            ...
            if (designerWorkload >= maxWorkload) {
                throw (0, ApiError_1.badRequest)('Designer has reached maximum workload');
            }
        ...
        }, transaction_1.TransactionConfig.DESIGNER_ASSIGNMENT);
    }
    /**
     * Unassign designer from draft
     */
    async unassignDesigner(draftId, unassignedBy, reason) {
        const transactionManager = new transaction_1.TransactionManager(this.prisma);
        return transactionManager.executeWithIsolation(async (tx) => {
            const draft = await tx.draft.findUnique({
                where: { id: draftId },
            ...
            if (draft.committedAt) {
                throw (0, ApiError_1.badRequest)('Cannot unassign designer from committed draft');
            }
            // Unassign designer
     ...
     */
    async getDesignerWorkload(designerId) {
     ...
     */
    async reassignDesigner(draftId, newDesignerId, reassignedBy, reason) {
        const transactionManager = new transaction_1.TransactionManager(this.prisma);
        return transactionManager.executeWithIsolation(async (tx) => {
            const draft = await tx.draft.findUnique({
                where: { id: draftId },
            ...
            if (draft.committedAt) {
                throw (0, ApiError_1.badRequest)('Cannot reassign designer for committed draft');
            }
            if (!draft.assignedDesignerId) {
                throw (0, ApiError_1.badRequest)('No designer currently assigned');
            }
            ...
            if (!newDesigner || newDesigner.roles.length === 0) {
                throw (0, ApiError_1.badRequest)('New user is not a designer');
            }
            ...
            if (newDesignerWorkload >= 5) {
                throw (0, ApiError_1.badRequest)('New designer has reached maximum workload');
            }
            // Reassign atomically
            await tx.draft.update({
                where: { id: draftId },
...
exports.DraftAssignmentService = DraftAssignmentService;
