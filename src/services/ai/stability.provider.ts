...
export class StabilityProvider {
  ...
  private mapModel(model?: string): string {
    ...
    if (!model) return 'sdxl-1.0';
    const m = model.toLowerCase();
    if (m.includes('sdxl')) return 'sdxl-1.0';
    return 'sdxl-1.0';
  }

  async generate(input: StabilityParams): Promise<Buffer[]> {
    if (!this.apiKey) throw new Error('STABILITY_API_KEY is required');
    ...
    if (input.negative) body.append('negative_prompt', input.negative);
    body.append('output_format', 'png');
    body.append('model', model);
    body.append('width', String(input.params.width));
    body.append('height', String(input.params.height));
    if (input.params.steps) body.append('steps', String(input.params.steps));
    if (input.params.guidance) body.append('guidance', String(input.params.guidance));
