...
exports.getLikeSummaryController = getLikeSummaryController;
...
function getRedisClient() {
    if (!redisClient) {
        redisClient = (0, connection_1.createRedisConnection)();
    }
    return redisClient;
}
async function invalidateCache(messageCardId) {
    try {
...
async function toggleLikeController(req, res) {
    ...
    const { messageCardId } = likes_dto_1.toggleLikeSchema.parse(req.body);
    // Check if message card exists
    const cardExists = await database_1.prisma.messageCard.findUnique({
    ...
    try {
        ...
        await database_1.prisma.like.delete({
            where: {
        ...
        if (error.code === 'P2025') {
            try {
                // Create the like
                await database_1.prisma.like.create({
...
async function getLikeSummaryController(req, res) {
    const { id } = likes_dto_1.likeSummaryParamsSchema.parse(req.params);
    ...
    let count = 0;
    try {
        count = await database_1.prisma.like.count({
